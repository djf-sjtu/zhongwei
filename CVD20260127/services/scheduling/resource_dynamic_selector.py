...
class ResourceSelector:
    ...
    def _select_tbs(self, wafer: SchedulingWafer) -> Optional[str]:
        """选择TBS (CVD: 不遍历layers，直接检查模块)"""

        angle = wafer.base.angle.value
        tbs_candidates = ConfigService.get_tbs_by_angle(angle)

        # 查找第一个可用的TBS (CVD: 直接检查模块，不遍历layers)
